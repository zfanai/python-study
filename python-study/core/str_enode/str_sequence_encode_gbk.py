...
def func4():
    str_a="周凡"
    print len(str_a)    # 结果是4, 也就是字符串编码(gbk)的长度
    print "\\x%02x" % ord(str_a[1])
    
    # 字符和数字是两种不同的类型，字符和数字指尖的转换关系是
    # 通过ord()函数和chr()，unichr()函数来架起它们之间的桥梁
    # 而字符也可以分为ascii字符和unicode字符两大类，而它们之间
    # 
    ...
    str_a=u"zf周凡"
    print len(str_a)    # 长度是4，Unicode编码
    
    print unicode("周凡", "gbk")    #正确
    #print unicode("周凡", "utf8")    #错误，"周凡"是gbk编码的，如果用utf8来解码就会出错

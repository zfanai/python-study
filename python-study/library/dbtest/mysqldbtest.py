...
def connect(db, dbName):
    ...
    if db == 'sqlite':
        ...
        try:
            ...
            cxn = MySQLdb.connect(user='root', passwd='myzfsql85')
            try:
                cxn.query('DROP DATABASE %s' % dbName)
            except DB_EXC.OperationalError, e:
                pass
            cxn.query('CREATE DATABASE %s' % dbName)
            cxn.query("GRANT ALL ON %s.* to 'root'@'localhost'" % dbName)
            cxn.commit()
            cxn.close()
            cxn = MySQLdb.connect(db=dbName, user='root', passwd='myzfsql85')
...
def func1():
    ...
    #print '*** Connecting to %r database' % db
    # 如果多个线程
    cxn = connect(db, 'test') # 一个连接就代表mysql数据库的一个连接。
    cxn1 = connect(db, 'test')   
    debug.trace(['cxn,cxn1:', cxn, cxn1])
    
    while raw_input('press q to quit.\n')=='q':
        break

# 两个线程同时访问一个数据库连接是会出错的。
def func2():
    db = setup()
    cxn = connect(db, 'test')
    cur = cxn.cursor()
